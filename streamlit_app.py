...
def initialize_ee():
    try:
        ...
        if not creds_dict:
            st.sidebar.error("GEE 인증 정보를 찾을 수 없습니다. Secret 설정을 확인하세요.")
            return False
...
# -------------------------
# RIGHT: GEE 기반 인터랙티브 지도
# -------------------------
with right_col:
    st.header("🗺️ 인터랙티브 시각화")
    ...
    if initialize_ee():
        ...
        st.error("🚨 GEE 인증에 실패했습니다. Secret 키 설정을 확인하고 앱을 재시작해주세요.")
